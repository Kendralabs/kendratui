...
pub use observability::{GovernanceConfig, ObservabilityConfig, SinkConfig};
...
pub struct AppConfig {
    ...
    // Hook configuration
    #[serde(default)]
    pub hooks: kendra_hooks::HookConfig,
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            ...
            hooks: kendra_hooks::HookConfig::empty(),
...
impl AppConfig {
    ...
    /// 5. `~/.kendra/auth.json` → `keys.<provider>` (stored via setup or TUI model picker)
    /// 6. `OPENAI_API_KEY` (last resort for truly unknown providers)
    pub fn get_api_key_with_env(&self, registry_env_var: Option<&str>) -> Result<String, String> {
        ...
        // Try auth.json → keys.<provider> (written by setup wizard or TUI model picker)
        let auth_key = std::env::var("KENDRA_DIR")
            .ok()
            ...
            .and_then(|val| {
                val.get("keys")
                    .and_then(|k| k.get(self.model_provider.to_lowercase().as_str()))
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string())
            });
