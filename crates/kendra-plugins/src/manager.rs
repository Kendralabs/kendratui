...
pub struct PluginPaths {
    /// Global plugins directory: ~/.kendra/plugins/
    pub global_plugins_dir: PathBuf,
    /// Project plugins directory: .kendra/plugins/
    pub project_plugins_dir: PathBuf,
    /// Global marketplaces directory: ~/.kendra/marketplaces/
    pub global_marketplaces_dir: PathBuf,
    /// Global plugin cache: ~/.kendra/plugins/cache/
...
impl PluginPaths {
    ...
    pub fn new(working_dir: Option<&Path>) -> Self {
        let paths = kendra_config::Paths::new(working_dir.map(PathBuf::from));
        let project_base = working_dir
            .map(|d| d.join(".kendra"))
            .unwrap_or_else(|| PathBuf::from(".kendra"));
...
impl PluginManager {
    ...
    pub fn load_manifest(&self, plugin_dir: &Path) -> Result<PluginManifest> {
        let possible_paths = [
            plugin_dir.join(".kendra").join("manifest.json"),
    ...
    pub fn load_plugin_metadata(&self, plugin_dir: &Path) -> Result<PluginMetadata> {
        let possible_paths = [
            plugin_dir.join(".kendra").join("plugin.json"),
            plugin_dir.join("plugin.json"),
        ];
