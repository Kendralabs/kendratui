...
#[test]
fn test_external_path_kendra_config_allowed() {
    let tmp = TempDir::new().unwrap();
    let wd = tmp.path().canonicalize().unwrap();
    if let Some(home) = dirs::home_dir() {
        assert!(!is_external_path(&home.join(".kendra/memory/test.md"), &wd));
    }
}
...
fn test_external_path_xdg_config_allowed() {
    ...
    if let Some(home) = dirs::home_dir() {
        assert!(!is_external_path(
            &home.join(".config/kendra/settings.toml"),
            &wd
        ));
    }
}
...
fn test_external_path_tmp_allowed() {
    let tmp = TempDir::new().unwrap();
    let wd = tmp.path().canonicalize().unwrap();
    assert!(!is_external_path(Path::new("/tmp/kendra-test.txt"), &wd));
}
...
fn test_external_path_var_tmp_allowed() {
    ...
    let wd = tmp.path().canonicalize().unwrap();
    assert!(!is_external_path(
        Path::new("/var/tmp/kendra-test.txt"),
        &wd
    ));
}
...
fn test_external_path_sibling_project() {
    ...
    let base = home.join(".kendra-test-workspace");
    let wd = base.join("my-project");
    std::fs::create_dir_all(&wd).unwrap();
    let wd = wd.canonicalize().unwrap();
    // Sibling project: resolves to ~/.kendra-test-workspace/other-project/main.rs
