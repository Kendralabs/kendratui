...
//!
//! Works with the `TodoManager` from `kendra-runtime` to let the agent
...
pub struct TodoTool {
    ...
    manager: Arc<Mutex<kendra_runtime::TodoManager>>,
}

impl TodoTool {
    /// Create a new todo tool with a shared manager.
    pub fn new(manager: Arc<Mutex<kendra_runtime::TodoManager>>) -> Self {
        Self { manager }
    }
}
