...
fn make_tool() -> (TodoTool, Arc<Mutex<kendra_runtime::TodoManager>>) {
    let mgr = Arc::new(Mutex::new(kendra_runtime::TodoManager::from_steps(&[
        "Step A".into(),
        "Step B".into(),
        "Step C".into(),
    ])));
...
async fn test_list_empty() {
    let mgr = Arc::new(Mutex::new(kendra_runtime::TodoManager::new()));
