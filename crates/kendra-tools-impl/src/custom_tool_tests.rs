...
fn test_discover_finds_manifests() {
    let tmp = tempfile::TempDir::new().unwrap();
    let tool_dir = tmp.path().join(".kendra").join("tools");
...
fn test_discover_deduplicates() {
    ...
    let dir1 = tmp.path().join(".kendra").join("tools");
...
fn test_resolve_command_relative() {
    ...
    let tool = CustomTool::new(manifest, PathBuf::from("/project/.kendra/tools"));
    assert_eq!(
        tool.resolve_command(),
        PathBuf::from("/project/.kendra/tools/run.sh")
    );
}
