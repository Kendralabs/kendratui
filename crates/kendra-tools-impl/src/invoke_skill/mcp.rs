        ...
        match mgr.get_prompt(server_name, prompt_name, prompt_args).await {
            Ok(result) => {
                ...
                for msg in &result.messages {
                    ...
                    match &msg.content {
                        kendra_mcp::models::McpPromptContent::Text(text) => {
                            output.push_str(text);
                        }
                        kendra_mcp::models::McpPromptContent::Structured { text } => {
                            output.push_str(text);
                        }
                        kendra_mcp::models::McpPromptContent::Multiple(blocks) => {
                            for block in blocks {
                                if let kendra_mcp::models::McpContent::Text { text } = block {
                                    output.push_str(text);
                                }
                            }
                        }
