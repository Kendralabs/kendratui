...
//! Channel configuration is loaded from `~/.kendra/settings.json`
//! or `.kendra/settings.json`.
            ...
            _ => {
                return ToolResult::fail(format!(
                    "No webhook URL configured for channel '{channel}'. \
                     Set it in ~/.kendra/settings.json under channels.{channel}.webhook_url \
...
fn load_channel_config() -> HashMap<String, serde_json::Value> {
    ...
    let config_paths = [
        Some(kendra_config::Paths::default().global_settings()),
        Some(PathBuf::from(".kendra").join("settings.json")),
    ];
