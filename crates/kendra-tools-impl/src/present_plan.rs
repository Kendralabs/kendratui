...
//! 2. Stores the plan file to `~/.kendra/plans/`
//!
...
use kendra_runtime::{PlanApprovalRequest, PlanApprovalSender, PlanIndex, TodoManager};
...
impl PresentPlanTool {
    ...
    /// Plans directory: `~/.kendra/plans/`.
    fn plans_dir() -> Option<PathBuf> {
        Some(kendra_config::Paths::default().global_plans_dir())
    }
}
...
impl BaseTool for PresentPlanTool {
    ...
    async fn execute(
        ...
        // Register in PlanIndex and copy plan to ~/.kendra/plans/
        let plan_name = if let Some(plans_dir) = Self::plans_dir() {
            let name = kendra_runtime::generate_plan_name(Some(&plans_dir), 50);
