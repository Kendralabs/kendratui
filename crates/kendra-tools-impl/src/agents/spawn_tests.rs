...
async fn test_spawn_subagent_missing_params() {
    let manager = Arc::new(kendra_agents::SubagentManager::new());
    let registry = Arc::new(kendra_tools_core::ToolRegistry::new());
    let raw = kendra_http::HttpClient::new(
        "https://api.example.com/v1/chat/completions",
        reqwest::header::HeaderMap::new(),
        None,
    )
    .unwrap();
    let http = Arc::new(kendra_http::AdaptedClient::new(raw));
...
async fn test_planner_blocked_during_explore_phase() {
    let mut manager = kendra_agents::SubagentManager::new();
    // Register a planner spec so agent_type validation passes
    manager.register(kendra_agents::SubAgentSpec::new(
        "Planner",
        "Plans tasks",
        "",
    ));
