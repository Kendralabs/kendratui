...
use kendra_tools_core::{BaseTool, ToolContext, ToolResult};
use tokio::sync::mpsc;
...
pub struct SpawnSubagentTool {
    ...
    manager: Arc<kendra_agents::SubagentManager>,
    /// Full tool registry (subagents filter to their allowed subset).
    tool_registry: Arc<kendra_tools_core::ToolRegistry>,
    /// HTTP client for LLM API calls.
    http_client: Arc<kendra_http::AdaptedClient>,
    ...
    debug_logger: Option<Arc<kendra_runtime::SessionDebugLogger>>,
...
impl SpawnSubagentTool {
    ...
    pub fn new(
        ...
        let cached_description = kendra_agents::prompts::embedded::TOOLS_TOOL_SPAWN_AGENT
    ...
    pub fn with_debug_logger(mut self, logger: Arc<kendra_runtime::SessionDebugLogger>) -> Self {
...
impl BaseTool for SpawnSubagentTool {
    ...
    async fn execute(
        ...
        let progress: Arc<dyn kendra_agents::SubagentProgressCallback> =
            if let Some(ref tx) = self.event_tx {
                ...
                Arc::new(kendra_agents::NoopProgressCallback)
            };
        ...
        match result {
            Ok(run_result) => {
                ...
                let child_mgr = kendra_history::SessionManager::new(self.session_dir.clone());
                if let Ok(child_mgr) = child_mgr {
                    let mut session = kendra_models::session::Session::new();
                    ...
                    let messages = kendra_history::message_convert::api_values_to_chatmessages(
                ...
                if content.len() > MAX_SUBAGENT_OUTPUT {
                    ...
                    output.push_str(kendra_runtime::safe_truncate(content, half));
                    output.push_str(&format!(
                        "\n\n[...truncated {} chars...]\n\n",
                ...
                if let Some(ref tx) = self.event_tx {
                    let _ = tx.send(SubagentEvent::Finished {
                        ...
                        result_summary: if content.len() > 200 {
                            format!("{}...", kendra_runtime::safe_truncate(content, 200))
                        } else {
                            content.clone()
                        },
...
impl SpawnSubagentTool {
    ...
    async fn spawn_background(&self, params: BackgroundSpawnParams<'_>) -> ToolResult {
        ...
        let interrupt_token = kendra_runtime::InterruptToken::new();
        ...
        tokio::spawn(async move {
            ...
            match result {
                Ok(run_result) => {
                    // Save child session for future resume
                    let child_mgr = kendra_history::SessionManager::new(session_dir);
                    if let Ok(child_mgr) = child_mgr {
                        let mut session = kendra_models::session::Session::new();
                    ...
                    let summary = if run_result.agent_result.content.len() > 200 {
                        format!(
                            "{}...",
                            kendra_runtime::safe_truncate(&run_result.agent_result.content, 200)
                        )
                    } else {
