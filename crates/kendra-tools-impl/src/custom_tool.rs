//! Custom tool loaded from `.kendra/tools/` directory.
//!
//! Users can define custom tools by placing a JSON manifest file alongside
//! an executable script in `.kendra/tools/` (or `.opencode/tool/`).
//!
...
pub fn discover_custom_tools(working_dir: &Path) -> Vec<CustomTool> {
    let search_dirs = [
        working_dir.join(".kendra").join("tools"),
