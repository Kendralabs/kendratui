...
//!
//! Activated when `kendra_DEBUG_EVENTS=1` is set. Records all [`AppEvent`]
//! variants to a JSONL file with sequence numbers and timestamps.

use std::io::Write;
use std::path::{Path, PathBuf};

use kendra_models::message::ChatMessage;
...
///
/// Activated when the `kendra_DEBUG_EVENTS=1` environment variable is set.
...
impl EventRecorder {
    ...
    /// Create a recorder if KENDRA_DEBUG_EVENTS=1 or kendra_DEBUG_EVENTS=1 is set.
    ///
    /// Writes to app_dir/debug/events-<timestamp>.jsonl.
    pub fn from_env() -> Option<Self> {
        let debug_enabled = std::env::var("KENDRA_DEBUG_EVENTS")
            .or_else(|_| std::env::var("kendra_DEBUG_EVENTS"))
            .ok()?
            .as_str()
            == "1";
        if !debug_enabled {
        ...
        let debug_dir = paths.global_dir().join("debug");
