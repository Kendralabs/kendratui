...
//!
//! Stores user input history to `~/.kendra/history.json` and supports
...
impl CommandHistory {
    /// Create a new command history, loading from `~/.kendra/history.json`
    /// if it exists.
    pub fn new() -> Self {
    ...
    fn default_path() -> PathBuf {
        ...
        home.join(".kendra").join("history.json")
    }
