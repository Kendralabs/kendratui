...
//! - File-based output storage (`/tmp/kendra/<path>/tasks/<id>.output`)
...
impl BackgroundTaskManager {
    ...
    ///
    /// `working_dir` is used to derive a unique output directory under `/tmp/kendra/`.
    ...
    fn get_output_dir(working_dir: &Path) -> PathBuf {
        ...
        PathBuf::from(format!("/tmp/kendra/{safe_path}/tasks"))
    }
...
impl Default for BackgroundTaskManager {
    fn default() -> Self {
        Self {
            tasks: HashMap::new(),
            handles: HashMap::new(),
            output_dir: PathBuf::from("/tmp/kendra/default/tasks"),
            listeners: Vec::new(),
        }
    }
}
