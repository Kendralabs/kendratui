...
impl WelcomePanelState {
    ...
    pub fn tick(&mut self, _terminal_width: u16, _terminal_height: u16) {
        ...
        // Braille offset advances every 4 ticks (slower)
        self.braille_tick += 1;
        if self.braille_tick >= 4 {
            self.braille_tick = 0;
    ...
    pub fn ensure_rain_field(&mut self, w: usize, h: usize) {
        ...
        for i in 0..w {
            if i % 3 != 0 {
                // Inactive rain column to reduce visual clutter and simplify animation
            ...
            let speed = 0.03 + pseudo_rand(&mut self.rng_seed) * 0.12;
            let trail_len = 3 + (pseudo_rand(&mut self.rng_seed) * 4.0) as u8;
    ...
    pub(super) fn step_rain(&mut self) {
        ...
        for col in &mut self.rain_columns {
            ...
            if col.y > h + col.trail_len as f32 {
                ...
                col.speed = 0.03 + (col.char_offset as f32 % 9.0) * 0.015;
            }
        }
    }
}
