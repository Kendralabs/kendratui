...
pub(super) fn map_todo_items(mgr: &kendra_runtime::TodoManager) -> Vec<TodoDisplayItem> {
    mgr.all()
        .iter()
        .map(|item| TodoDisplayItem {
            ...
            status: match item.status {
                kendra_runtime::TodoStatus::Pending => TodoDisplayStatus::Pending,
                kendra_runtime::TodoStatus::InProgress => TodoDisplayStatus::InProgress,
                kendra_runtime::TodoStatus::Completed => TodoDisplayStatus::Completed,
            },
    ...
    pub(super) fn handle_event(&mut self, event: AppEvent) {
        ...
        match event {
            ...
            AppEvent::ToolApprovalRequested {
                ...
                diff,
                response_tx,
            } => self.handle_tool_approval_requested(command, working_dir, diff, response_tx),
