...
impl App {
        ...
        if matches!(tool_name.as_str(), "Agent" | "spawn_subagent") {
            ...
            self.state.messages.push(DisplayMessage {
                ...
                tool_call: Some(DisplayToolCall {
                    ...
                    nested_calls: Vec::new(),
                    error_text: if success { None } else { Some(String::new()) },
                }),
                collapsed: false,
                thinking_started_at: None,
                thinking_duration_secs: None,
    ...
    pub(super) fn handle_tool_approval_required(&mut self, description: String) {
        ...
        let _rx = self.approval_controller.start(description, wd, None);
        self.state.dirty = true;
    }
    ...
    pub(super) fn handle_tool_approval_requested(
        ...
        working_dir: String,
        diff: Option<String>,
        response_tx: tokio::sync::oneshot::Sender<kendra_runtime::ToolApprovalDecision>,
    ) {
        // Map AutonomyLevel to PermissionMode
        use kendra_runtime::PermissionMode;
        let mode = match self.state.autonomy {
            AutonomyLevel::Auto => PermissionMode::Auto,
            AutonomyLevel::SemiAuto => PermissionMode::Default,
            AutonomyLevel::Manual => PermissionMode::DontAsk,
        };

        // Decision based on PermissionMode
        let auto_approve = match mode {
            PermissionMode::Auto | PermissionMode::BypassPermissions => true,
            PermissionMode::Default => kendra_runtime::is_safe_command(&command),
            _ => false,
        };

        if auto_approve {
            let _ = response_tx.send(kendra_runtime::ToolApprovalDecision {
                approved: true,
                choice: "yes".to_string(),
                command,
            });
        } else {
            let _rx = self.approval_controller.start(command, working_dir, diff);
