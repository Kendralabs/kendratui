...
use kendra_config::models_dev::{ModelInfo, ModelRegistry};
...
impl ModelPickerController {
    ...
    pub fn from_registry_with_custom(
        ...
        Self::from_registry_with_custom_provider(
            cache_dir,
            current_model,
            custom_models,
            "custom",
    ...
    pub fn from_registry_with_custom_provider(
        ...
        let get_configured_keys = || {
            let mut keys = std::collections::HashSet::new();
            let auth_path = kendra_config::Paths::new(None)
                .config_dir()
                .join("auth.json");
            let parsed_keys = std::fs::read_to_string(auth_path)
                .ok()
                .and_then(|content| serde_json::from_str::<serde_json::Value>(&content).ok())
                .and_then(|val| {
                    val.get("keys")
                        .and_then(|k| k.as_object())
                        .map(|obj| obj.keys().map(|k| k.to_lowercase()).collect::<Vec<_>>())
                });
            if let Some(keys_vec) = parsed_keys {
                for key in keys_vec {
                    keys.insert(key);
                }
            }
        ...
        providers.sort_by_key(|p| {
            let has_key = p.api_key_env.is_empty()
                ...
                || configured_keys.contains(&p.id.to_lowercase());
            !has_key // false < true, so has_key=true sorts first
        });
        for provider in &providers {
            let has_api_key = provider.api_key_env.is_empty()
                || std::env::var(&provider.api_key_env).is_ok()
                || configured_keys.contains(&provider.id.to_lowercase());
