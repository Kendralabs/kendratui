...
/// not in well-known config locations (`~/.kendra/`, `~/.config/kendra/`, `/tmp/`).
...
pub fn is_external_path(resolved: &Path, working_dir: &Path) -> bool {
    ...
    if let Some(home) = dirs::home_dir() {
        // Always allow legacy ~/.kendra and XDG ~/.config/kendra
        let allowed = [home.join(".kendra"), home.join(".config").join("kendra")];
