...
fn test_env_override_model_provider() {
    ...
    ConfigLoader::apply_env_overrides_with(
        &mut config,
        mock_env(&[("kendra_MODEL_PROVIDER", "anthropic")]),
    );
...
fn test_env_override_model() {
    let mut config = AppConfig::default();
    ConfigLoader::apply_env_overrides_with(&mut config, mock_env(&[("kendra_MODEL", "gpt-4o")]));
...
fn test_env_override_max_tokens() {
    let mut config = AppConfig::default();
    ConfigLoader::apply_env_overrides_with(&mut config, mock_env(&[("kendra_MAX_TOKENS", "8192")]));
...
fn test_env_override_max_tokens_invalid_ignored() {
    ...
    ConfigLoader::apply_env_overrides_with(
        &mut config,
        mock_env(&[("kendra_MAX_TOKENS", "not_a_number")]),
    );
...
fn test_env_override_temperature() {
    let mut config = AppConfig::default();
    ConfigLoader::apply_env_overrides_with(&mut config, mock_env(&[("kendra_TEMPERATURE", "0.9")]));
...
fn test_env_override_temperature_invalid_ignored() {
    ...
    ConfigLoader::apply_env_overrides_with(&mut config, mock_env(&[("kendra_TEMPERATURE", "hot")]));
...
fn test_env_override_verbose() {
    ...
    ConfigLoader::apply_env_overrides_with(&mut config, mock_env(&[("kendra_VERBOSE", "true")]));
    assert!(config.verbose);

    config.verbose = false;
    ConfigLoader::apply_env_overrides_with(&mut config, mock_env(&[("kendra_VERBOSE", "1")]));
    assert!(config.verbose);

    config.verbose = true;
    ConfigLoader::apply_env_overrides_with(&mut config, mock_env(&[("kendra_VERBOSE", "false")]));
    assert!(!config.verbose);
}
...
fn test_env_override_debug() {
    ...
    ConfigLoader::apply_env_overrides_with(&mut config, mock_env(&[("kendra_DEBUG", "0")]));
    assert!(!config.debug_logging);

    // And back on
    ConfigLoader::apply_env_overrides_with(&mut config, mock_env(&[("kendra_DEBUG", "TRUE")]));
    assert!(config.debug_logging);
}
...
fn test_custom_models_load_from_config() {
    ...
    assert_eq!(
        config.custom_models.len(),
        2,
        "custom_models should have 2 entries, got: {:?}",
        config.custom_models
    );
    assert_eq!(
        config.custom_models[0].id,
        "@cf/meta/llama-3.3-70b-instruct-fp8-fast"
    );
    assert_eq!(config.custom_models[0].name, "Llama 3.3 70B FP8 Fast (CF)");
    assert_eq!(
        config.custom_models[1].id,
...
fn test_substitute_env_variable() {
    unsafe { std::env::set_var("kendra_TEST_SUB_VAR", "test-value-123") };
    let input = r#"{"api_key": "{env:kendra_TEST_SUB_VAR}"}"#;
    let result = ConfigLoader::substitute_templates(input, Path::new("."));
    assert_eq!(result, r#"{"api_key": "test-value-123"}"#);
    unsafe { std::env::remove_var("kendra_TEST_SUB_VAR") };
}

#[test]
fn test_substitute_env_missing_variable() {
    let input = r#"{"key": "{env:kendra_NONEXISTENT_9999}"}"#;
    let result = ConfigLoader::substitute_templates(input, Path::new("."));
...
fn test_substitute_multiple_tokens() {
    unsafe { std::env::set_var("kendra_TEST_MULTI_A", "val-a") };
    unsafe { std::env::set_var("kendra_TEST_MULTI_B", "val-b") };
    let input = r#"{"a": "{env:kendra_TEST_MULTI_A}", "b": "{env:kendra_TEST_MULTI_B}"}"#;
    let result = ConfigLoader::substitute_templates(input, Path::new("."));
    assert_eq!(result, r#"{"a": "val-a", "b": "val-b"}"#);
    unsafe { std::env::remove_var("kendra_TEST_MULTI_A") };
    unsafe { std::env::remove_var("kendra_TEST_MULTI_B") };
}
...
fn test_substitute_env_then_load() {
    let tmp = TempDir::new().unwrap();
    unsafe { std::env::set_var("kendra_TEST_PROVIDER_SUB", "anthropic") };

    let config_file = tmp.path().join("config.json");
    std::fs::write(
        &config_file,
        r#"{"model_provider": "{env:kendra_TEST_PROVIDER_SUB}"}"#,
    )
    .unwrap();
    ...
    unsafe { std::env::remove_var("kendra_TEST_PROVIDER_SUB") };
}
