//! Centralized path management for KendraCLI.
...
pub const APP_DIR_NAME: &str = ".kendra";
pub const LEGACY_APP_DIR_NAME: &str = ".opendev";
...
pub const ENV_KENDRA_DIR: &str = "KENDRA_DIR";
pub const ENV_kendra_DIR: &str = "kendra_DIR";
pub const ENV_OPENDEV_DIR: &str = "OPENDEV_DIR";
pub const ENV_opendev_DIR: &str = "opendev_DIR";

pub const ENV_KENDRA_SESSION_DIR: &str = "KENDRA_SESSION_DIR";
pub const ENV_kendra_SESSION_DIR: &str = "kendra_SESSION_DIR";
pub const ENV_OPENDEV_SESSION_DIR: &str = "OPENDEV_SESSION_DIR";
pub const ENV_opendev_SESSION_DIR: &str = "opendev_SESSION_DIR";

pub const ENV_KENDRA_LOG_DIR: &str = "KENDRA_LOG_DIR";
pub const ENV_kendra_LOG_DIR: &str = "kendra_LOG_DIR";
pub const ENV_OPENDEV_LOG_DIR: &str = "OPENDEV_LOG_DIR";
pub const ENV_opendev_LOG_DIR: &str = "opendev_LOG_DIR";

pub const ENV_KENDRA_CACHE_DIR: &str = "KENDRA_CACHE_DIR";
pub const ENV_kendra_CACHE_DIR: &str = "kendra_CACHE_DIR";
...
/// - Project paths (<working_dir>/.kendra/...)
...
impl Paths {
    ...
    pub fn new(working_dir: Option<PathBuf>) -> Self {
        ...
        // Full override via environment variable (Kendra priority)
        let override_dir = std::env::var(ENV_KENDRA_DIR)
            .or_else(|_| std::env::var(ENV_kendra_DIR))
            .or_else(|_| std::env::var(ENV_OPENDEV_DIR))
            .or_else(|_| std::env::var(ENV_opendev_DIR))
            .ok();
        ...
        // Try ~/.kendra first, then fallback to ~/.opendev
        ...
        if app_dir.exists() {
            return Self {
                ...
                cache_dir: app_dir.join(CACHE_DIR_NAME),
                state_dir: app_dir,
            };
        }

        if legacy_dir.exists() {
            // Attempt one-time migration from legacy to new directory
            if let Err(e) = std::fs::rename(&legacy_dir, &app_dir) {
                warn!(
                    "Failed to migrate legacy directory {:?} to {:?}: {}",
                    legacy_dir, app_dir, e
                );
                // Fallback to using legacy dir directly if rename fails
                return Self {
                    working_dir,
            ...
            info!(
                "Successfully migrated legacy directory {:?} to {:?}",
                legacy_dir, app_dir
            );
            return Self {
                working_dir,
        ...
        if let Some(proj_dirs) = ProjectDirs::from("", "", "kendra") {
            Self {
                working_dir,
            ...
            let base = PathBuf::from("/tmp").join("kendra");
            return Self {
                working_dir,
                config_dir: base.clone(),
                data_dir: base.clone(),
    ...
    pub fn global_sessions_dir(&self) -> PathBuf {
        if let Ok(override_dir) = std::env::var(ENV_KENDRA_SESSION_DIR)
            .or_else(|_| std::env::var(ENV_kendra_SESSION_DIR))
            .or_else(|_| std::env::var(ENV_OPENDEV_SESSION_DIR))
            .or_else(|_| std::env::var(ENV_opendev_SESSION_DIR))
        {
    ...
    pub fn global_logs_dir(&self) -> PathBuf {
        if let Ok(override_dir) = std::env::var(ENV_KENDRA_LOG_DIR)
            .or_else(|_| std::env::var(ENV_kendra_LOG_DIR))
            .or_else(|_| std::env::var(ENV_OPENDEV_LOG_DIR))
            .or_else(|_| std::env::var(ENV_opendev_LOG_DIR))
        {
            return PathBuf::from(override_dir);
        }
    ...
    pub fn global_cache_dir(&self) -> PathBuf {
        if let Ok(override_dir) = std::env::var(ENV_KENDRA_CACHE_DIR)
            .or_else(|_| std::env::var(ENV_kendra_CACHE_DIR))
            .or_else(|_| std::env::var(ENV_OPENDEV_CACHE_DIR))
            .or_else(|_| std::env::var(ENV_opendev_CACHE_DIR))
        {
            return PathBuf::from(override_dir);
        }
    ...
    /// Get project-level kendra directory.
    ...
    ///
    /// Used by `is_external_path` to allow access to all kendra directories.
