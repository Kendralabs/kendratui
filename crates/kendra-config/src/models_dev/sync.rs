...
pub fn sync_provider_cache(
    ...
    if std::env::var("KENDRA_DISABLE_REMOTE_MODELS")
        .or_else(|_| std::env::var("kendra_DISABLE_REMOTE_MODELS"))
        .is_ok_and(|v| matches!(v.to_lowercase().as_str(), "1" | "true" | "yes"))
    {
        return Ok(false);
    }

    // Support KENDRA_MODELS_DEV_PATH override
    let catalog = if let Ok(override_path) =
        std::env::var("KENDRA_MODELS_DEV_PATH").or_else(|_| std::env::var("kendra_MODELS_DEV_PATH"))
    {
        ...
        if path.exists() {
            ...
            warn!("KENDRA_MODELS_DEV_PATH {:?} does not exist", path);
            return Ok(false);
        }
    } else {
...
async fn fetch_models_dev_async() -> Option<serde_json::Value> {
    let client = reqwest::Client::builder()
        ...
        .user_agent(
            std::env::var("KENDRA_HTTP_USER_AGENT")
                .or_else(|_| std::env::var("kendra_HTTP_USER_AGENT"))
                .unwrap_or_else(|_| "kendra-rust/0.1.0".to_string()),
        )
        .build()
        .ok()?;
