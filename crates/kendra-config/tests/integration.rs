...
use kendra_config::{ConfigLoader, ModelInfo, ModelRegistry, ProviderInfo};
...
fn config_skill_paths_single_layer() {
    ...
    std::fs::write(&global, r#"{"skill_paths": ["~/.kendra/extra-skills"]}"#).unwrap();

    let config = ConfigLoader::load(&global, &project).unwrap();
    assert_eq!(config.skill_paths, vec!["~/.kendra/extra-skills"]);
}
