...
//!
//! Mirrors `kendra/repl/repl.py::_handle_command`.

use kendra_config::ModelRegistry;
use kendra_runtime::AutonomyLevel;
...
impl BuiltinCommands {
    ...
    ///
    /// If `cache_dir` is `None`, uses the default `~/.kendra/cache` directory.
    ...
    pub fn handle_model_picker(
        ...
        if registry.providers.is_empty() {
            println!("No models available. Run `kendra setup` or check network connectivity.");
            return Vec::new();
        }
    ...
    fn handle_sound(&self) {
        kendra_runtime::play_finish_sound();
