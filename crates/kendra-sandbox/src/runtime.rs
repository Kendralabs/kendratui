...
const BUNDLED_MSB_DIR: &str = ".kendra/runtime/msb";
...
///
/// Search order:
/// 1. Bundled with kendra at `~/.kendra/runtime/msb/bin/msb`
/// 2. Homebrew libexec (macOS): `$(brew --prefix kendra)/libexec/msb/bin/msb`
/// 3. System PATH
...
pub async fn ensure_server_running() -> Result<()> {
    ...
    let msb = find_msb_binary().ok_or_else(|| {
        SandboxError::ServerUnavailable(
            "msb binary not found. Reinstall kendra or run: \
