...
use kendra_channels::telegram::RemoteSessionBridge;
use kendra_channels::telegram::remote::{
            ...
            while let Some(req) = original_approval_rx.recv().await {
                ...
                match remote_rx.await {
                    Ok(response) => match response {
                        kendra_channels::telegram::remote::ApprovalResponse::Approved {
                            command,
                        } => {
                        ...
                        kendra_channels::telegram::remote::ApprovalResponse::Denied => {
                            let _ = req.response_tx.send(kendra_runtime::ToolApprovalDecision {
                                approved: false,
                                choice: "no".to_string(),
