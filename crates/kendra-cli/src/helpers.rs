...
pub fn init_tracing(verbose: bool, tui_mode: bool) {
    ...
    if tui_mode {
        ...
        {
            let log_dir = kendra_config::Paths::default().global_logs_dir();
            let _ = std::fs::create_dir_all(&log_dir);
            let log_path = log_dir.join("kendra.log");
...
pub fn load_app_config(working_dir: &std::path::Path) -> kendra_models::AppConfig {
    ...
    match kendra_config::ConfigLoader::load(&global_settings, &project_settings) {
        Ok(config) => config,
        Err(e) => {
            eprintln!("Warning: failed to load config: {e}");
            kendra_models::AppConfig::default()
        }
    }
}

/// Load the merged MCP configuration (global + project).
pub fn load_mcp_config(working_dir: &std::path::Path) -> kendra_mcp::McpConfig {
    let paths = kendra_config::Paths::new(Some(working_dir.to_path_buf()));
...
pub fn save_global_mcp_config(config: &kendra_mcp::McpConfig) {
...
/// Install a custom panic hook that writes crash reports to `~/.kendra/crash/`.
pub fn install_panic_handler() {
    ...
    std::panic::set_hook(Box::new(move |panic_info| {
        ...
        report.push_str("kendra Crash Report\n");
        ...
        // Write crash report to app_dir/crash/
        let paths = kendra_config::Paths::default();
        let crash_dir = paths.global_dir().join("crash");
        if let Ok(()) = std::fs::create_dir_all(&crash_dir) {
            ...
            if std::fs::write(&crash_path, &report).is_ok() {
                eprintln!(
                    "\nKendraCLI crashed unexpectedly. A crash report has been saved to:\n  {}\n\nPlease include this file when reporting the issue.\n",
                    crash_path.display()
                );
            } else {
                eprintln!("\nKendraCLI crashed unexpectedly. Failed to write crash report.\n");
            }
        }
