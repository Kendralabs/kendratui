...
use kendra_models::TelegramChannelConfig;

use crate::cli::*;
use crate::helpers::*;

/// Convert model DmPolicy to channel DmPolicy.
fn to_channel_dm_policy(p: &kendra_models::DmPolicy) -> kendra_channels::telegram::DmPolicy {
    match p {
        kendra_models::DmPolicy::Open => kendra_channels::telegram::DmPolicy::Open,
        kendra_models::DmPolicy::Pairing => kendra_channels::telegram::DmPolicy::Pairing,
        kendra_models::DmPolicy::Allowlist => kendra_channels::telegram::DmPolicy::Allowlist,
    }
}
...
pub async fn handle_channel(action: ChannelAction, working_dir: &std::path::Path) {
    match action {
        ChannelAction::Add { token } => {
            ...
            eprint!("Validating...");
            let api = kendra_channels::telegram::api::TelegramApi::new(bot_token.clone());
            ...
            let paths = kendra_config::Paths::default();
            ...
            config.channels.telegram = Some(TelegramChannelConfig {
                ...
                dm_policy: kendra_models::DmPolicy::Pairing,
        ...
        ChannelAction::Status => {
            ...
            match &config.channels.telegram {
                Some(tg) if tg.enabled => {
                    let api =
                        kendra_channels::telegram::api::TelegramApi::new(tg.bot_token.clone());
                    match api.get_me().await {
                        Ok(user) => {
                            println!(
                                "telegram  @{}  {:?}",
                ...
                None => {
                    println!("No channel configured. Run: kendra channel add");
                }
            }
        }
        ChannelAction::Serve { foreground } => {
            if foreground {
                ...
                let pid_path = kendra_config::Paths::default()
                    .global_dir()
        ...
        ChannelAction::Pair { user_id } => {
            ...
            {
                let api = kendra_channels::telegram::api::TelegramApi::new(bot_token);
                let _ = api
                    .send_message(kendra_channels::telegram::types::SendMessageRequest {
                        chat_id,
...
async fn run_telegram_serve(working_dir: &std::path::Path) {
    ...
    let router = std::sync::Arc::new(kendra_channels::MessageRouter::new());
    ...
    match kendra_channels::telegram::start_telegram(Some(&telegram_config), router).await {
...
pub fn require_telegram(config: &kendra_models::AppConfig) -> TelegramChannelConfig {
    match &config.channels.telegram {
        ...
        None => {
            eprintln!("Error: no channel configured. Run: kendra channel add");
            std::process::exit(1);
        }
    }
}

/// Handle the `kendra remote` subcommand — headless Telegram remote session.
...
fn save_config(config: &kendra_models::AppConfig, path: &std::path::Path) {
    if let Err(e) = kendra_config::ConfigLoader::save(config, path) {
