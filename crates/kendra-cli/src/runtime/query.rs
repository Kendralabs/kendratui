...
use kendra_agents::traits::{AgentError, AgentEventCallback, AgentResult};
use kendra_history::topic_detector::SimpleMessage;
use kendra_mcp::McpManager;
use kendra_models::message::{ChatMessage, Role};
use kendra_tools_core::ToolContext;
...
impl AgentRuntime {
    ...
    ///
    /// Loads MCP config from global (`~/.kendra/mcp.json`) and project
    ...
    pub async fn run_query(
        ...
        interrupt_token: Option<&kendra_runtime::InterruptToken>,
        plan_requested: bool,
        ...
        let outcome = self
            .hook_manager
            .run_hooks(
                kendra_hooks::HookEvent::UserPromptSubmit,
                None,
        ...
        let query_owned;
        let query = if let Some(serde_json::Value::String(ref updated)) = outcome.updated_input {
            query_owned = updated.clone();
            &query_owned
        } else {
            query
        };
        ...
        let session_messages = self
            .session_manager
            .current_session()
            .map(|s| kendra_history::message_convert::chatmessages_to_api_values(&s.messages))
            .unwrap_or_default();
        ...
        let shared_state = if plan_requested {
            let plans_dir = dirs_next::home_dir()
                .map(|h: std::path::PathBuf| h.join(".kendra").join("plans"))
                .unwrap_or_else(|| std::path::PathBuf::from("/tmp"));
            let plan_name = kendra_runtime::generate_plan_name(Some(&plans_dir), 50);
            let plan_path = format!("~/.kendra/plans/{}.md", plan_name);
        ...
        if plan_requested {
            let plan_path = shared_state
                ...
                .unwrap_or_default();
            let reminder = kendra_agents::prompts::reminders::get_reminder(
            ...
            && session.messages.len() > 1
        {
            ...
            let plans_dir = Some(kendra_config::Paths::default().global_plans_dir());
            if let Some(ref plans_dir) = plans_dir {
                let plan_index = kendra_runtime::PlanIndex::new(plans_dir);
            ...
            && let Ok(mut mgr) = self.snapshot_manager.lock()
        {
                ...
                && pre_hash != post_hash
            {
                ...
                if !stats.is_empty() {
                    ...
                    if let Some(session) = self.session_manager.current_session_mut() {
                        use kendra_models::file_change::{FileChange, FileChangeType};
                        for stat in &stats {
        ...
        // so tool calls and their results are fully preserved.
        {
            ...
            let new_chat_messages =
                kendra_history::message_convert::api_values_to_chatmessages(new_values);
        ...
        if let Ok(idx) = self.artifact_index.lock()
            && !idx.is_empty()
        {
            let paths = kendra_config::paths::Paths::new(Some(self.working_dir.clone()));
            let fh_path = paths.project_file_history();
            let mut global =
                kendra_context::ArtifactIndex::load_from_file(&fh_path).unwrap_or_default();
            global.merge(&idx);
    ...
    pub async fn inject_background_result(
        ...
        let mut tool_call = kendra_models::message::ToolCall {
    ...
    pub async fn run_compaction(&mut self) -> Result<String, String> {
        use kendra_agents::prompts::embedded::SYSTEM_COMPACTION;
        ...
        let compacted = if let Some((payload, _middle_count, keep_recent)) = build_result {
            ...
            let summary = match summary_text {
                ...
                _ => {
                    ...
                    if let Ok(comp) = self.compactor.lock() {
                        comp.fallback_summary(
        ...
        let compacted_values: Vec<serde_json::Value> = compacted
            ...
            .collect();
        let new_chat_messages =
            kendra_history::message_convert::api_values_to_chatmessages(&compacted_values);
