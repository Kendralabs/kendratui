...
use kendra_agents::llm_calls::{LlmCallConfig, LlmCaller};
use kendra_agents::react_loop::{ReactLoop, ReactLoopConfig};
use kendra_context::{ArtifactIndex, ContextCompactor};
use kendra_history::SessionManager;
use kendra_history::topic_detector::TopicDetector;
use kendra_http::HttpClient;
use kendra_http::adapted_client::AdaptedClient;
use kendra_http::adapters::base::ProviderAdapter;
use kendra_mcp::McpManager;
use kendra_models::AppConfig;
use kendra_repl::HandlerRegistry;
use kendra_repl::query_enhancer::QueryEnhancer;
use kendra_runtime::{CostTracker, SessionDebugLogger};
use kendra_tools_core::{BaseTool, ToolRegistry};
use kendra_tools_impl::*;
...
pub struct AgentRuntime {
    ...
    pub todo_manager: Arc<Mutex<kendra_runtime::TodoManager>>,
    /// Tool approval sender — passed to react loop for gating bash execution.
    pub tool_approval_tx: Option<kendra_runtime::ToolApprovalSender>,
    ...
    pub(super) skill_loader: Arc<Mutex<kendra_agents::SkillLoader>>,
    ...
    pub(super) snapshot_manager: Arc<Mutex<kendra_history::SnapshotManager>>,
    /// Per-session debug logger for LLM interactions (noop when debug_logging is off).
    pub debug_logger: Arc<SessionDebugLogger>,
    /// Governed orchestrator for durable phase-gated execution.
    pub orchestrator: Arc<Mutex<kendra_runtime::orchestration::GovernedOrchestrator>>,
    /// Prompt composer for per-turn system prompt composition with section caching.
    pub prompt_composer: Mutex<kendra_agents::prompts::PromptComposer>,
    /// Prompt context (runtime values for conditional section inclusion).
    pub prompt_context: Mutex<kendra_agents::prompts::PromptContext>,
    /// Hook manager for lifecycle events.
    pub hook_manager: Arc<kendra_hooks::HookManager>,
}

/// Receivers returned from tool registration for TUI bridging.
pub struct ToolChannelReceivers {
    pub ask_user_rx: kendra_runtime::AskUserReceiver,
    pub plan_approval_rx: kendra_runtime::PlanApprovalReceiver,
    pub tool_approval_rx: kendra_runtime::ToolApprovalReceiver,
    pub subagent_event_rx:
        Option<tokio::sync::mpsc::UnboundedReceiver<kendra_tools_impl::SubagentEvent>>,
}
...
impl AgentRuntime {
    ...
    pub fn new(
        ...
        let overflow_dir = working_dir.join(".kendra").join("tool-output");
        // Clean up overflow files older than 7 days on startup.
        kendra_tools_core::cleanup_overflow_dir(&overflow_dir);
        ...
        // Register custom tools from .kendra/tools/ and .opencode/tool/ directories.
        let custom_tools = kendra_tools_impl::custom_tool::discover_custom_tools(working_dir);
        ...
        let git_root = kendra_agents::git_root(working_dir);

        // Register invoke_skill tool with project-local and user-global skill dirs.
        // Scans .kendra/skills at each level from working_dir up to git root,
...

        {
            ...
            loop {
                skill_dirs.push(current.join(".kendra").join("skills"));
        ...
        if let Some(home) = dirs_next::home_dir() {
            skill_dirs.push(home.join(".kendra").join("skills"));
        }
        ...
        let mut skill_loader_inner = kendra_agents::SkillLoader::new(skill_dirs);
        ...
        let paths = kendra_config::Paths::new(Some(working_dir.to_path_buf()));
        let registry = kendra_config::ModelRegistry::load_from_cache(&paths.global_cache_dir());
        ...
        let (api_url, headers, adapter): (
            String,
            HeaderMap,
            Option<Box<dyn kendra_http::adapters::base::ProviderAdapter>>,
        ) = match provider.as_str() {
            "anthropic" => {
                let adapter = kendra_http::adapters::anthropic::AnthropicAdapter::new();
                ...
                (
                    url,
                    hdrs,
                    Some(Box::new(adapter) as Box<dyn kendra_http::adapters::base::ProviderAdapter>),
                )
            }
            "openai" => {
                // OpenAI uses /v1/responses (Responses API) with Bearer auth
                let adapter = kendra_http::adapters::openai::OpenAiAdapter::new();
                let url = effective_base_url.unwrap_or_else(|| adapter.api_url().to_string());
                let mut hdrs = HeaderMap::new();
            ...
            "ollama" => {
                let adapter = kendra_http::adapters::ollama::OllamaAdapter::new();
                let url = effective_base_url.unwrap_or_else(|| adapter.api_url().to_string());
                let mut hdrs = HeaderMap::new();
                hdrs.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
                (
            ...
            "gemini" | "google" => {
                let adapter = kendra_http::adapters::gemini::GeminiAdapter::new(&config.model);
                let api_url = effective_base_url
                    .map(|base| kendra_http::adapters::gemini::gemini_api_url(&base, &config.model))
                    .unwrap_or_else(|| {
                        kendra_http::adapters::gemini::gemini_api_url(
                            adapter.api_url(),
                            &config.model,
                        )
                    });
            ...
            "azure" => {
                ...
                hdrs.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
                let adapter = kendra_http::adapters::chat_completions::ChatCompletionsAdapter::new(
                    url.clone(),
                );
                (
                    url,
            ...
            provider => {
                ...
                if provider == "openrouter"
                    && let Ok(val) = HeaderValue::from_str("https://kendra.ai")
                {
        ...
        let circuit_breaker =
            std::sync::Arc::new(kendra_http::CircuitBreaker::with_defaults(&provider));
        ...
        let orchestrator = Arc::new(Mutex::new(
            kendra_runtime::orchestration::GovernedOrchestrator::new(
                session_id.clone(),
                working_dir,
                session_manager.session_dir().to_path_buf(),
            ),
        ));

        // Initialize HookManager
        let mut hook_config = config.hooks.clone();
        hook_config.compile_all();
        hook_config.strip_unknown_events();
        let hook_manager = Arc::new(kendra_hooks::HookManager::new(
            hook_config,
            session_id,
        ...
        let mut subagent_manager =
            kendra_agents::SubagentManager::with_builtins_and_custom_git_root(
                working_dir,
                git_root.as_deref(),
            );
        // Apply inline agent config overrides from kendra.json
        ...
        let (subagent_event_tx, subagent_event_rx) =
            tokio::sync::mpsc::unbounded_channel::<kendra_tools_impl::SubagentEvent>();
        ...
        let runtime = Self {
            ...
            skill_loader,
            topic_detector,
            snapshot_manager: Arc::new(Mutex::new(kendra_history::SnapshotManager::new(
        ...
        // Trigger SessionStart hook in the background
        runtime.hook_manager.run_hooks_async(
            kendra_hooks::HookEvent::SessionStart,
...
impl AgentRuntime {
    ...
    pub fn switch_model(&mut self, new_model: &str) -> Result<String, String> {
        ...
        let paths = kendra_config::Paths::new(Some(self.working_dir.clone()));
        let registry = kendra_config::ModelRegistry::load_from_cache(&paths.global_cache_dir());
        ...
        let (new_provider_id, new_model_info) =
            if let Some((provider_id, _key, model_info)) = registry.find_model_by_id(new_model) {
                ...
                let api_key = self.config.get_api_key_with_env(None).unwrap_or_default();
                if !api_key.is_empty() {
    ...
    fn build_http_client(
        ...
        let (api_url, headers, adapter): (String, HeaderMap, Option<Box<dyn ProviderAdapter>>) =
            match provider {
                ...
                "gemini" | "google" => {
                    let adapter = kendra_http::adapters::gemini::GeminiAdapter::new(model);
                    let api_url = api_base_url
                        .map(|base| kendra_http::adapters::gemini::gemini_api_url(base, model))
                        .unwrap_or_else(|| {
                            kendra_http::adapters::gemini::gemini_api_url(adapter.api_url(), model)
                        });
                    let mut hdrs = HeaderMap::new();
                    if let Ok(val) = HeaderValue::from_str(api_key) {
                        hdrs.insert("x-goog-api-key", val);
                ...
                "azure" => {
                    ...
                    hdrs.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
                    let adapter =
                        kendra_http::adapters::chat_completions::ChatCompletionsAdapter::new(
                            url.clone(),
                        );
                    (
                        url,
...
impl Drop for AgentRuntime {
    fn drop(&mut self) {
        self.hook_manager
            .run_hooks_async(kendra_hooks::HookEvent::SessionEnd, None, None);
    }
}
