...
use kendra_agents::prompts::create_default_composer;
use kendra_tools_core::ToolRegistry;
...
pub(super) fn register_default_tools(
    ...
    Arc<Mutex<kendra_runtime::TodoManager>>,
    ToolChannelReceivers,
    kendra_runtime::ToolApprovalSender,
) {
    ...
    let (ask_user_tx, ask_user_rx) = kendra_runtime::ask_user_channel();
    ...
    let todo_manager = Arc::new(Mutex::new(kendra_runtime::TodoManager::new()));

    // Plan tool — with channel for TUI approval AND todo manager
    let (plan_approval_tx, plan_approval_rx) = kendra_runtime::plan_approval_channel();
    ...
    if !display_map.is_empty() {
        kendra_tui::formatters::tool_registry::init_runtime_display(display_map);
    }

    // Tool approval channel (sender stored on runtime for react loop, receiver goes to TUI)
    let (tool_approval_tx, tool_approval_rx) = kendra_runtime::tool_approval_channel();
...
pub fn create_prompt_composer(
    working_dir: &Path,
    config: &kendra_models::AppConfig,
) -> (
    kendra_agents::prompts::PromptComposer,
    kendra_agents::prompts::PromptContext,
) {
    ...
    composer.register_dynamic_section(
        "mcp_instructions",
        kendra_agents::prompts::CachePolicy::Uncached,
...
#[cfg(test)]
pub fn build_system_prompt(working_dir: &Path, config: &kendra_models::AppConfig) -> String {
