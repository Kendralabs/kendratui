...
use chrono::Utc;
use kendra_agents::traits::AgentEventCallback;
use kendra_history::SessionManager;
use kendra_models::message::{ChatMessage, Role};
use kendra_runtime::InterruptToken;
use kendra_tui::app::AppState;
use kendra_tui::{App, AppEvent, ExitInfo};

use kendra_channels::telegram::remote::{RemoteCommandReceiver, RemoteEvent, RemoteEventSender};
...
pub struct TuiRunner {
    ...
    remote_bridge: Option<Arc<kendra_channels::telegram::RemoteSessionBridge>>,
}
...
impl TuiRunner {
    ...
    pub fn with_remote_control(
        ...
        bridge: Arc<kendra_channels::telegram::RemoteSessionBridge>,
    ) -> Self {
            ...
            while let Some(msg) = user_rx.recv().await {
                ...
                if msg == "\x00__SHARE__" {
                    ...
                    if let Some(session) = runtime.session_manager.current_session() {
                        match kendra_history::sharing::share_session(session, "").await {
                            Ok(url) => {
                        ...
                        // it from re-spawning the same agents on the next user message.
                        {
                            ...
                            if !new_values.is_empty() {
                                let new_chat_messages =
                                    kendra_history::message_convert::api_values_to_chatmessages(
                                        new_values,
                                    );
                                        ...
                                        Ok(http_result) if http_result.success => {
                                            if let Some(body) = http_result.body {
                                                ...
                                                if let Some(ref usage_json) = response.usage {
                                                    let token_usage =
                                                        kendra_runtime::TokenUsage::from_json(
                                                            usage_json,
                                                        );
                                    ...
                                    tokio::spawn(async move {
                                        ...
                                        match result {
                                            Ok(r) => {
                                                ...
                                                let full_result =
                                                    kendra_runtime::build_background_result(
                                                        &r.content,
                                                        &r.messages,
                                                        12000,
                                                    );
