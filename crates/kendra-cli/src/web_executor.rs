...
use kendra_agents::AgentEventCallback;
use kendra_web::state::{AppState, WsBroadcast};
use uuid::Uuid;
...
pub struct WebEventCallback {
    state: AppState,
    session_id: String,
    todo_manager: Option<std::sync::Arc<std::sync::Mutex<kendra_runtime::TodoManager>>>,
}
            ...
            let todos: Vec<serde_json::Value> = mgr
                ...
                .map(|item| {
                    let status = match item.status {
                        kendra_runtime::TodoStatus::Pending => "pending",
                        kendra_runtime::TodoStatus::InProgress => "in_progress",
                        kendra_runtime::TodoStatus::Completed => "completed",
                    };
...
pub fn spawn_channel_bridges(receivers: crate::runtime::ToolChannelReceivers, state: AppState) {
    ...
    if let Some(mut subagent_rx) = receivers.subagent_event_rx {
        ...
        tokio::spawn(async move {
            use kendra_tools_impl::SubagentEvent;
            ...
            while let Some(req) = ask_rx.recv().await {
                ...
                let rx = st
                    .add_pending_ask_user(
                        request_id,
                        kendra_web::state::PendingAskUser {
            ...
            while let Some(req) = tool_rx.recv().await {
                ...
                let rx = st
                    .add_pending_approval(
                        approval_id,
                        kendra_web::state::PendingApproval {
                ...
                match rx.await {
                    Ok(result) => {
                        ...
                        let _ = req.response_tx.send(kendra_runtime::ToolApprovalDecision {
            ...
            while let Some(req) = plan_rx.recv().await {
                ...
                let rx = st
                    .add_pending_plan_approval(
                        request_id,
                        kendra_web::state::PendingPlanApproval {
                ...
                match rx.await {
                    Ok(result) => {
                        let _ = req.response_tx.send(kendra_runtime::PlanDecision {
...
impl kendra_web::state::AgentExecutor for WebAgentExecutor {
    async fn execute_query(
        ...
        // are saved under the web session's ID.
        {
            ...
            if needs_load && rt.session_manager.resume_session(&session_id).is_err() {
                ...
                let mut session = kendra_models::Session::new();
        ...
        let plan_requested = state.mode().await == kendra_web::state::OperationMode::Plan;
