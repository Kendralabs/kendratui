...
fn create_event_store(session_dir: &std::path::Path) -> kendra_history::event_store::EventStore {
    let event_bus = kendra_runtime::event_bus::EventBus::new();
    let bridge = kendra_runtime::event_bus::create_event_bus_bridge(event_bus);
    kendra_history::event_store::EventStore::new(session_dir.to_path_buf()).with_post_append(bridge)
}
...
pub async fn run_non_interactive(
    ...
    use kendra_history::{SessionListing, SessionManager};
    ...
    // Apply profile overrides (from kendra_PROFILE env var)
    if let Ok(profile) =
        std::env::var("KENDRA_PROFILE").or_else(|_| std::env::var("kendra_PROFILE"))
    {
        kendra_config::apply_profile(&mut config, &profile);
    }
...
pub async fn run_interactive(
    ...
    // Apply profile overrides (from --profile flag or kendra_PROFILE env var)
    let effective_profile = profile_name.or_else(|| {
        std::env::var("KENDRA_PROFILE")
            .or_else(|_| std::env::var("kendra_PROFILE"))
            .ok()
    });
    ...
    if continue_session {
        ...
        match resume_id {
            ...
            None => {
                ...
                let paths_for_listing = kendra_config::Paths::new(Some(working_dir.to_path_buf()));
                let sessions =
    ...
    let resolved_theme = theme_name
        .as_deref()
        .and_then(kendra_tui::ThemeName::from_str_loose)
        .unwrap_or_else(kendra_tui::auto_detect_theme);
    ...
    let mut app_state = kendra_tui::AppState {
        model: config.model.clone(),
        path_shortener: kendra_tui::formatters::PathShortener::new(Some(&wd_str)),
        working_dir: wd_str,
        git_branch: detect_git_branch(working_dir),
        version: env!("CARGO_PKG_VERSION").to_string(),
        theme: resolved_theme.theme(),
        theme_name: resolved_theme,
        reasoning_level: kendra_tui::app::ReasoningLevel::from_str_loose(&config.reasoning_effort),
        session_id,
        ..kendra_tui::AppState::default()
    };
        ...
        for msg in &session.messages {
            match msg.role {
                kendra_models::Role::User => {
                    ...
                    if kendra_models::message::is_system_injected_content(&msg.content) {
                        continue;
                    }
                    app_state
                        .messages
                        .push(kendra_tui::app::DisplayMessage::new(
                            kendra_tui::app::DisplayRole::User,
                            msg.content.clone(),
                        ));
                }
                kendra_models::Role::Assistant => {
                    ...
                    let trace = msg
                        ...
                        .unwrap_or("");
                    if !trace.is_empty() {
                        app_state.messages.push(kendra_tui::app::DisplayMessage {
                            role: kendra_tui::app::DisplayRole::Reasoning,
                    ...
                    if !msg.content.is_empty() {
                        app_state
                            .messages
                            .push(kendra_tui::app::DisplayMessage::new(
                                kendra_tui::app::DisplayRole::Assistant,
                                msg.content.clone(),
                            ));
                    }
                    ...
                    for tc in &msg.tool_calls {
                        ...
                        app_state.messages.push(kendra_tui::app::DisplayMessage {
                            role: kendra_tui::app::DisplayRole::Assistant,
                            content: String::new(),
                            tool_call: Some(kendra_tui::app::DisplayToolCall::from_model(tc)),
                            collapsed: false,
                            thinking_started_at: None,
                ...
                kendra_models::Role::System => {} // Skip system messages
            }
        }
    }
...

    {
        ...
        if tg_config.as_ref().is_some_and(|tg| tg.enabled) {
            ...
            let telegram_config = kendra_channels::telegram::TelegramConfig {
                bot_token: tg.bot_token.clone(),
                enabled: true,
                group_mention_only: tg.group_mention_only,
                dm_policy: match tg.dm_policy {
                    kendra_models::DmPolicy::Open => kendra_channels::telegram::DmPolicy::Open,
                    kendra_models::DmPolicy::Pairing => {
                        kendra_channels::telegram::DmPolicy::Pairing
                    }
                    kendra_models::DmPolicy::Allowlist => {
                        kendra_channels::telegram::DmPolicy::Allowlist
                    }
                },
                allowed_users: tg.allowed_users.clone(),
            };

            match kendra_channels::telegram::start_telegram_remote(Some(&telegram_config)).await {
...
fn print_exit_message(exit_info: &kendra_tui::ExitInfo) {
...
pub async fn run_replay(path: &std::path::Path) {
    use kendra_tui::event::load_recorded_events;
