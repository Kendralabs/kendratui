...
/// User-global permissions path: `~/.kendra/permissions.json`.
pub(crate) fn user_permissions_path() -> Option<PathBuf> {
    Some(
        kendra_config::Paths::default()
            .config_dir()
...
pub(crate) fn load_persistent_rules(rules: &mut Vec<ApprovalRule>, project_dir: Option<&Path>) {
    ...
    if let Some(dir) = project_dir {
        load_rules_from_file(rules, &dir.join(".kendra").join("permissions.json"));
    }
}
...
pub(crate) fn save_persistent_rules(
    ...
    let path = match scope {
        ...
        RuleScope::Project => project_dir.map(|d| d.join(".kendra").join("permissions.json")),
        RuleScope::All => {
            // Save to both
