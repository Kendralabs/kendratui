...
//!
//! Stores a lightweight JSON index at `~/.kendra/plans/plans-index.json`
//! following atomic-write patterns (tempfile + rename).
//!
//! Ported from `kendra/core/runtime/plan_index.py`.
...
impl PlanIndex {
    ...
    ///
    /// # Arguments
    /// * `plans_dir` - Directory containing plan files (e.g. `~/.kendra/plans/`).
