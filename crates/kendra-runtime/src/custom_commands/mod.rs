//! Custom commands loaded from `.kendra/commands/`.
//!
...
//! # Example
//!
//! `.kendra/commands/review.md` contains:
//! ```text
//! ---
...
impl CustomCommandLoader {
    ...
    ///
    /// Scans `.kendra/commands/` under the project directory (higher priority)
    ...
    ///
    /// Searches `.kendra/commands/` at project and global levels.
    fn get_command_dirs(&self) -> Vec<(PathBuf, &'static str)> {
        let mut dirs = Vec::new();

        // Project-local commands (highest priority)
        let local = self.working_dir.join(".kendra/commands");
        if local.is_dir() {
        ...
        if let Some(home) = dirs_next::home_dir() {
            let global = home.join(".kendra/commands");
            if global.is_dir() {
