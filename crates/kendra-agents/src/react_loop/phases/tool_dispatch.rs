...
use kendra_context::ArtifactIndex;
use kendra_runtime::{
    TodoManager, TodoStatus, extract_command_prefix, play_finish_sound, summarize_tool_result,
};
use kendra_tools_core::parallel::{ParallelPolicy, ToolCall as ParallelToolCall};
use kendra_tools_core::{ToolContext, ToolRegistry, ToolResult};
...
fn generate_diff_preview(file_path: &str, original: &str, modified: &str) -> String {
    ...
    for edit in edits {
        match edit {
            Edit::Keep(oi, _) => {
                lines.push(format!(" {}", original_lines[oi]));
                orig_count += 1;
                mod_count += 1;
            }
            Edit::Remove(oi) => {
            ...
            Edit::Add(mi) => {
                lines.push(format!("+{}", modified_lines[mi]));
                mod_count += 1;
            }
        }
    }

    output.push(format!("@@ -1,{} +1,{} @@", orig_count, mod_count));
    output.extend(lines);
    output.join("\n")
}
...
pub(in crate::react_loop) async fn execute_sequential<M>(
    ...
    tool_approval_tx: Option<&kendra_runtime::ToolApprovalSender>,
    orchestrator: Option<&Mutex<kendra_runtime::orchestration::GovernedOrchestrator>>,
    hook_manager: Option<&kendra_hooks::HookManager>,
) -> Option<LoopAction>
where
    M: TaskMonitor + ?Sized,
{
    ...
    for tc in tool_calls {
        ...
        let mut args_map =
            kendra_tools_core::normalizer::normalize_params(tool_name, args_map, Some(&wd_str));

        let tool_call_id_str = tc.get("id").and_then(|id| id.as_str()).unwrap_or("unknown");

        // Hook execution: PreToolUse
        let mut hook_permission_allows = false;
        let mut hook_permission_denied = false;
        let mut hook_block_reason = String::new();

        if let Some(hook_mgr) = hook_manager {
            let event_data = serde_json::json!({
                "tool_input": args_value,
            });
            let outcome = hook_mgr
                .run_hooks(
                    kendra_hooks::HookEvent::PreToolUse,
                    Some(tool_name),
                    Some(&event_data),
                )
                .await;
            ...
            if outcome.blocked {
                hook_permission_denied = true;
                hook_block_reason = format!("Blocked by PreToolUse hook: {}", outcome.block_reason);
            } else {
                if let Some(ref dec) = outcome.permission_decision {
                    if dec == "allow" {
                        hook_permission_allows = true;
                    } else if dec == "deny" {
                        hook_permission_denied = true;
                        hook_block_reason = "Permission denied by PreToolUse hook".to_string();
                    }
                }

                if let Some(Value::Object(map)) = outcome.updated_input {
                    for (k, v) in map {
                        args_map.insert(k, v);
                    }
                }
        ...
        if hook_permission_denied {
            let result_content = ReactLoop::format_tool_result(
                tool_name,
                &serde_json::json!({
                    "success": false,
                    "error": hook_block_reason
                }),
            );
            ...
            emitter.emit_tool_result(tool_call_id_str, tool_name, &hook_block_reason, false);
        ...
        // Governed Orchestration: Checkpoint before file write
        ...
        if is_file_edit_tool {
            if let Some(orchestrator) = orchestrator {
                if let Ok(mut orch) = orchestrator.lock() {
                    ...
                    orch.create_checkpoint(&[&file_path]);
                }
            }
        }

        // Permission enforcement
        let mut permission_allows = hook_permission_allows;
        if !permission_allows && !react_loop.config.permission.is_empty() {
                ...
                match action {
                    ...
                    PermissionAction::Ask => {
                            ...
                            && let Some(approval_tx) = tool_approval_tx
                        {
                            ...
                            let req = kendra_runtime::ToolApprovalRequest {
            ...
            && let Some(approval_tx) = tool_approval_tx
        {
            ...
            let diff = if let (Some(original), Some(modified)) = (
                args_map.get("original").and_then(|v| v.as_str()),
                args_map.get("content").and_then(|v| v.as_str()),
            ) {
                Some(generate_diff_preview(&file_path, original, modified))
            } else {
                None
            };
        ...
        if let Some(hook_mgr) = hook_manager {
            let event = if tool_result.success {
                kendra_hooks::HookEvent::PostToolUse
            } else {
                kendra_hooks::HookEvent::PostToolUseFailure
            };
            let event_data = serde_json::json!({
                "tool_input": args_value,
                "tool_response": if tool_result.success {
                    serde_json::json!({
                        "success": true,
            ...
            let outcome = hook_mgr
                .run_hooks(event, Some(tool_name), Some(&event_data))
                .await;

            if let Some(ref add_ctx) = outcome.additional_context {
                if tool_result.success {
                    let mut current_output = tool_result.output.unwrap_or_default();
                    if !current_output.is_empty() {
                        current_output.push_str("\n\n");
                    }
                    current_output.push_str("Additional Context from hook:\n");
                    current_output.push_str(add_ctx);
                    tool_result.output = Some(current_output);
                } else {
                    let mut current_error = tool_result.error.unwrap_or_default();
                    if !current_error.is_empty() {
                        current_error.push_str("\n\n");
                    }
                    current_error.push_str("Additional Context from hook:\n");
                    current_error.push_str(add_ctx);
                    tool_result.error = Some(current_error);
                }
            }
        }
