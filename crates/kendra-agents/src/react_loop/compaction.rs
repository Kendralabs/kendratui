...
use kendra_context::{ArtifactIndex, ContextCompactor, OptimizationLevel};
use kendra_http::adapted_client::AdaptedClient;
use kendra_tools_core::ToolResult;
...
pub(super) fn values_to_api_messages(
    values: &[Value],
) -> Vec<kendra_context::compaction::ApiMessage> {
    values
        .iter()
...
pub(super) async fn do_llm_compaction(
    ...
    let summary = match summary_text {
        ...
        _ => {
            ...
            if let Ok(comp) = compactor.lock() {
                comp.fallback_summary(&api_msgs[1..api_msgs.len().saturating_sub(keep_recent)])
            } else {
                String::new()
            }
        }
