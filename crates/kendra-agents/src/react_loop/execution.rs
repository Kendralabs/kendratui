...
use kendra_runtime::{CostTracker, SessionDebugLogger, TodoManager};
use kendra_tools_core::{ToolContext, ToolRegistry};
...
impl ReactLoop {
    ...
    pub async fn run<M>(
        ...
        orchestrator: Option<&Mutex<kendra_runtime::GovernedOrchestrator>>,
        ...
        loop {
            ...
            if let Some(orchestrator) = orchestrator {
                if let Ok(mut orch) = orchestrator.lock() {
                    // Sync with shared_state planning_phase
                    if let Some(shared) = tool_context.shared_state.as_ref() {
                        if let Ok(shared_state) = shared.lock() {
                            if let Some(p) =
                                shared_state.get("planning_phase").and_then(|v| v.as_str())
                            {
                                match p {
                                    "explore" => orch.transition(
                                        kendra_models::orchestration::OrchestrationPhase::Explore,
                                    ),
                                    "plan" => orch.transition(
                                        kendra_models::orchestration::OrchestrationPhase::Plan,
                                    ),
                                    _ => {}
                                }
                            }
                    ...
                    match orch.phase() {
                        kendra_models::orchestration::OrchestrationPhase::Explore => {
                            // Phase tracking happens in track_exploration_tools
                        }
                        kendra_models::orchestration::OrchestrationPhase::Plan => {
                            // Logic to ensure the agent is focused on planning
                        }
                        kendra_models::orchestration::OrchestrationPhase::Work => {
                            // Work phase is managed by tool execution hooks
                        }
                        kendra_models::orchestration::OrchestrationPhase::Validate => {
                            // Inject a "Validation Request" as a user message
                            // to force the assistant to verify the work.
                            inject_system_message(
                                messages,
                                "Task completed. Please verify that the implementation matches the plan and requirements. Check for edge cases and regressions.",
                                MessageClass::Nudge,
                            );
                        }
                    }
            ...
            match turn {
                ...
                TurnResult::Complete { content, status } => {
                    ...
                    match super::phases::handle_completion(
                        ...
                        super::types::LoopAction::Return(result) => {
                            if let Some(hook_mgr) = hook_manager {
                                if let Ok(ref res) = result {
                                    hook_mgr.run_hooks_async(
                                        kendra_hooks::HookEvent::Stop,
                                        None,
                                        Some(serde_json::json!({
                                            "agent_result": res.content,
                                        })),
                                    );
                                }
                            }
    ...
    pub(super) fn track_exploration_tools(
        tool_context: &kendra_tools_core::ToolContext,
        tool_names: &[String],
        messages: &mut Vec<Value>,
    ) {
