...
pub(super) struct LoopState {
    ...
    pub subdir_tracker: kendra_context::SubdirInstructionTracker,
...
impl LoopState {
    ...
    pub fn new(working_dir: &std::path::Path) -> Self {
        let startup_paths: Vec<PathBuf> = kendra_context::discover_instruction_files(working_dir)
            .into_iter()
            .map(|f| f.path)
            .collect();
        let subdir_tracker = kendra_context::SubdirInstructionTracker::new(
