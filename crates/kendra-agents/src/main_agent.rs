...
//!
//! Mirrors `kendra/core/agents/main_agent/agent.py`.
...
impl BaseAgent for MainAgent {
    ...
    fn build_tool_schemas(&self) -> Vec<Value> {
        ...
        if let Some(ref provider) = self.config.model_provider {
            kendra_http::adapters::adapt_for_provider(&self.tool_schemas, provider)
        } else {
    ...
    async fn run(
        ...
        self.react_loop
            .run(
                ...
                _deps.orchestrator.as_ref().map(|o| o.as_ref()),
                None, // debug_logger: not wired through MainAgent
                None, // hook_manager
            )
            .await
    }
