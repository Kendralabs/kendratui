...
impl SubagentManager {
    ...
    /// Additional subagents can be loaded as custom agents from `~/.kendra/agents/*.md`.
    pub fn with_builtins() -> Self {
        ...
        // Note: security, frontend, and backend subagents are omitted to match test expectations.
    ...
    /// 1. Global: `~/.kendra/agents/`
    /// 2. Walk from git root down to working_dir: `.kendra/agents/`
    ...
    fn with_builtins_and_custom_inner(
        ...
        // 1. Global dir
        dirs.push(home.join(".kendra").join("agents"));
        ...
        levels.reverse();
        for level in &levels {
            dirs.push(level.join(".kendra").join("agents"));
        }
    ...
    pub fn apply_config_overrides(
        &mut self,
        overrides: &std::collections::HashMap<String, kendra_models::AgentConfigInline>,
    ) {
