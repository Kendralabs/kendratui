...
fn test_with_builtins_and_custom() {
    ...
    let agent_dir = tmp.path().join(".kendra").join("agents");
...
#[test]
fn test_only_kendra_agents_dir_loaded() {
    let tmp = tempfile::tempdir().unwrap();

    // Create agents in .kendra, .agents, and .claude dirs
    let kendra_dir = tmp.path().join(".kendra").join("agents");
    let agents_dir = tmp.path().join(".agents").join("agents");
    let claude_dir = tmp.path().join(".claude").join("agents");
    std::fs::create_dir_all(&kendra_dir).unwrap();
    std::fs::create_dir_all(&agents_dir).unwrap();
    std::fs::create_dir_all(&claude_dir).unwrap();

    std::fs::write(
        kendra_dir.join("shared.md"),
        "---\ndescription: From .kendra\n---\nkendra version.",
    )
    .unwrap();
    std::fs::write(
    ...
    // Only .kendra is loaded
    assert_eq!(spec.description, "From .kendra");
...
fn test_config_override_model() {
    ...
    overrides.insert(
        "build".to_string(),
        kendra_models::AgentConfigInline {
