...
/// Tools available to the Security subagent.
pub const SECURITY_TOOLS: &[&str] = &["Read", "Grep", "Glob", "web_search", "osv_scan"];

/// Create the Security subagent spec.
pub fn security(system_prompt: &str) -> SubAgentSpec {
    SubAgentSpec::new(
        "Security",
        "Security expert focused on vulnerability research and audits. \
         USE FOR: Identifying security risks in code or dependencies, \
         running security scans, and auditing trust boundaries.",
        system_prompt,
    )
    .with_tools(SECURITY_TOOLS.iter().map(|s| s.to_string()).collect())
}

/// Create the Frontend subagent spec.
pub fn frontend(system_prompt: &str) -> SubAgentSpec {
    SubAgentSpec::new(
        "Frontend",
        "Frontend expert specializing in UI/UX, React, and modern CSS. \
         USE FOR: Implementing or refactoring user interfaces, \
         polishing UI components, and ensuring frontend quality.",
        system_prompt,
    )
    // No .with_tools() — inherits all parent tools
}

/// Create the Backend subagent spec.
pub fn backend(system_prompt: &str) -> SubAgentSpec {
    SubAgentSpec::new(
        "Backend",
        "Backend expert specializing in Rust, Axum, and API design. \
         USE FOR: Implementing robust backend services, data modeling, \
         and ensuring backend performance and safety.",
        system_prompt,
    )
    // No .with_tools() — inherits all parent tools
}
