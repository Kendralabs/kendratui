...
use kendra_runtime::{Mailbox, SessionDebugLogger, ToolApprovalSender};
...
fn inject_mailbox_messages(msgs: Vec<kendra_runtime::MailboxMessage>, messages: &mut Vec<Value>) {
    for msg in msgs {
        let content = match msg.msg_type {
            kendra_runtime::MessageType::ShutdownRequest => format!(
                "[TEAM SHUTDOWN REQUEST from '{}']: {}\n\
                 Wrap up your current work and call task_complete.",
                msg.from, msg.content
            ),
            kendra_runtime::MessageType::Text => {
