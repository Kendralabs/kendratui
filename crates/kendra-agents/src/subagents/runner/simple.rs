...
///
/// Uses the canonical normalizer from `kendra-tools-core` so subagent event
...
fn normalize_tool_args(
    ...
    kendra_tools_core::normalizer::normalize_params(tool_name, args, Some(&wd))
}
...
impl SimpleReactRunner {
    ...
    async fn final_summary_call(
        ...
        let noop_cb = kendra_http::streaming::FnStreamCallback(|_| {});
        match ctx
            .http_client
                                ...
                                Ok(d) => {
                                    if d.choice == "yes_remember" {
                                        if matches!(name.as_str(), "Bash" | "run_command") {
                                            let prefix = kendra_runtime::extract_command_prefix(
                                                d.command.trim(),
                                            );
                                            debug!(
                                                prefix = %prefix,
