...
fn test_expand_variables() {
    ...
    vars.insert("project".to_string(), "KendraCLI".to_string());
    let result = SkillLoader::expand_variables(content, &vars);
    assert_eq!(result, "Hello Alice, welcome to KendraCLI.");
}
...
fn test_discover_skills_from_commands_dir() {
    let tmp = TempDir::new().unwrap();
    let kendra_dir = tmp.path().join(".kendra");
    let skills_dir = kendra_dir.join("skills");
    let commands_dir = kendra_dir.join("commands");
...
// ---- Only .kendra/skills is scanned ----

#[test]
fn test_discover_skills_from_kendra_skills_dir() {
    let tmp = TempDir::new().unwrap();
    let skills_dir = tmp.path().join(".kendra").join("skills");
    fs::create_dir_all(&skills_dir).unwrap();

    fs::write(
        skills_dir.join("my-tool.md"),
        "---\nname: my-tool\ndescription: A tool from .kendra/skills\n---\n\n# My Tool\n",
    )
    .unwrap();
