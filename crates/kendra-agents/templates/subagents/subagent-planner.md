...
- Start the app (`kendra` or `npm run dev` etc.) and exercise the changed feature
...
When your plan is written to the file, call **TaskStop** with a brief summary that includes the plan_file_path (e.g., "Plan written to ~/.kendra/plans/add-auth.md. Approach: ...").
