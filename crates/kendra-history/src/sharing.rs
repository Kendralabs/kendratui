...
fn render_session_html(session: &Session) -> String {
    ...
    for msg in &session.messages {
        let role_class = match msg.role {
            kendra_models::Role::User => "user",
            kendra_models::Role::Assistant => "assistant",
            kendra_models::Role::System => "system",
        };
...
pub async fn share_session(session: &Session, endpoint: &str) -> Result<String, String> {
    ...
    if endpoint.is_empty() {
        ...
        let filename = format!("kendra-session-{}.html", anonymized.id);
