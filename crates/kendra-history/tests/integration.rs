...
use chrono::Utc;
use kendra_history::{FileLock, SessionIndex, SessionListing, SessionManager, SnapshotManager};
use kendra_models::{ChatMessage, Role, Session};
...
fn with_file_lock_executes_closure() {
    let tmp = tempfile::NamedTempFile::new().unwrap();
    let result =
        kendra_history::file_locks::with_file_lock(tmp.path(), Duration::from_secs(5), || 42 + 1)
            .unwrap();
    assert_eq!(result, 43);
}
...
/// Snapshot diff ignores kendra internal overflow artifacts.
#[test]
fn snapshot_diff_ignores_internal_tool_output() {
    ...
    std::fs::write(tmp.path().join(".gitignore"), ".kendra/\n").unwrap();
    ...
    let tool_output_dir = tmp.path().join(".kendra").join("tool-output");
