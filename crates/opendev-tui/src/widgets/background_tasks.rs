...
impl Widget for TaskWatcherPanel<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let hint = format!(
            " j/k:nav  x:kill  d:dismiss  Enter:focus({focus_indicator})  Ctrl+D/U:scroll  Ctrl+P/q:close "
        );
...
mod tests {
    ...
    #[test]
    fn test_watcher_panel_rect_centering() {
        let area = Rect::new(0, 0, 100, 40);
        let panel = TaskWatcherPanel::panel_rect(area);
        assert!(panel.x > 0);
        assert!(panel.y > 0);
