...
use kendra_models::{AppConfig, Session};
use kendra_web::server::build_app;
use kendra_web::state::AppState;
...
fn make_test_state() -> (TempDir, AppState) {
    ...
    let user_store = kendra_http::UserStore::new(tmp.path().to_path_buf()).unwrap();
...
async fn health_check_returns_ok() {
    ...
    assert_eq!(json["service"], "kendra-web-ui");
}
...
async fn state_ws_broadcast() {
    ...
    state.broadcast(kendra_web::state::WsBroadcast::new(
        "test_event",
