...
fn make_state_with_workdir(work_dir: &str) -> AppState {
    ...
    let user_store = kendra_http::UserStore::new(tmp_path).unwrap();
    let model_registry = kendra_config::ModelRegistry::new();
    AppState::new(
        session_manager,
        config,
        work_dir.to_string(),
...
async fn test_list_servers_empty() {
    ...
    // May or may not have servers depending on user's ~/.kendra/mcp.json
...
async fn test_create_and_get_server() {
    let tmp = TempDir::new().unwrap();
    // Use a temp dir as working dir so we don't write to real ~/.kendra/
    ...
    // Create the .kendra directory.
    std::fs::create_dir_all(tmp.path().join(".kendra")).unwrap();
