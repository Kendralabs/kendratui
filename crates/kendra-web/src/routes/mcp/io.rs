...
/// Get the global MCP config path (~/.kendra/mcp.json).
pub(super) fn global_config_path() -> PathBuf {
    let home = std::env::var("HOME")
        .or_else(|_| std::env::var("USERPROFILE"))
        .unwrap_or_else(|_| "/tmp".to_string());
    PathBuf::from(home).join(".kendra").join("mcp.json")
}

/// Get the project-level MCP config path (.kendra/mcp.json in working_dir).
pub(super) fn project_config_path(working_dir: &str) -> PathBuf {
    PathBuf::from(working_dir).join(".kendra").join("mcp.json")
}
