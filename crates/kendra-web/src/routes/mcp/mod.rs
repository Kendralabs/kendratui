...
///
/// Loads the server configuration, creates a transport via `kendra_mcp`,
...
async fn connect_server(
    ...
    // Build an kendra_mcp::McpServerConfig from the web-layer config.
    let mcp_config = kendra_mcp::McpServerConfig {
    ...
    let manager = kendra_mcp::McpManager::new(Some(PathBuf::from(state.working_dir())));
    manager
