...
#[test]
fn test_discover_kendra_instructions() {
    let dir = TempDir::new().unwrap();
    let dir_path = dir.path().canonicalize().unwrap();
    std::fs::create_dir(dir_path.join(".kendra")).unwrap();
    std::fs::write(
        dir_path.join(".kendra/instructions.md"),
        "Custom instructions",
    )
    .unwrap();
    std::fs::create_dir(dir_path.join(".git")).unwrap();
...
#[test]
fn test_only_kendra_instructions_loaded() {
    let dir = TempDir::new().unwrap();
    let dir_path = dir.path().canonicalize().unwrap();
    std::fs::create_dir_all(dir_path.join(".kendra")).unwrap();
    std::fs::create_dir_all(dir_path.join(".claude")).unwrap();
    std::fs::write(dir_path.join(".kendra/instructions.md"), "kendra rules").unwrap();
    std::fs::write(dir_path.join(".claude/instructions.md"), "Claude rules").unwrap();
    std::fs::create_dir(dir_path.join(".git")).unwrap();

    let files = discover_instruction_files(&dir_path);
    assert_eq!(files.len(), 1);
    assert!(files[0].content.contains("kendra rules"));
}
