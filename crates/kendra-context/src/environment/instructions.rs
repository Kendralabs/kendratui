...
/// `.kendra/instructions.md` in each directory,
/// and global config at `~/.kendra/instructions.md` and `~/.config/kendra/AGENTS.md`.
///
...
pub fn discover_instruction_files(working_dir: &Path) -> Vec<InstructionFile> {
    ...
    loop {
        ...
        // Check .kendra/instructions.md
        let kendra_instr = current.join(".kendra").join("instructions.md");
        try_add_instruction(&kendra_instr, &current, working_dir, &mut files, &mut seen);
    ...
    if let Some(home) = dirs_next::home_dir() {
        let global_paths = [
            home.join(".kendra").join("instructions.md"),
            home.join(".kendra").join("AGENTS.md"),
            home.join(".config").join("kendra").join("AGENTS.md"),
        ];
...
fn try_add_instruction(
    ...
    let scope = if dir == working_dir || dir.starts_with(working_dir) {
        "project".to_string()
    } else if dir.to_string_lossy().contains(".kendra") || dir.to_string_lossy().contains(".config")
    {
        "global".to_string()
    } else {
        "parent".to_string()
