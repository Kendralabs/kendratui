...
impl WorktreeManager {
    ...
    pub fn create(&self, prefix: &str) -> Result<WorktreeInfo, String> {
        let branch_name = format!("kendra/{prefix}/{}", generate_short_id());
    ...
    /// Clean up all kendra-created worktrees.
    pub fn cleanup_all(&self) -> Vec<String> {
        ...
        for wt in worktrees {
            ...
            if wt.branch.starts_with("kendra/") {
    ...
    fn worktrees_dir(&self) -> PathBuf {
        self.repo_root.join(".git").join("kendra-worktrees")
    }
