...
use super::structural::StructuralContext;
...
pub struct ContextCompactor {
    ...
    pub structural_context: Option<StructuralContext>,
}
...
impl ContextCompactor {
    ...
    /// Set structural context for semantic-aware compaction.
    pub fn set_structural_context(&mut self, context: StructuralContext) {
        self.structural_context = Some(context);
    }
    ...
    /// Estimate total tokens across messages, system prompt, and structural context.
    ///
    /// Uses the improved `count_tokens` heuristic (cl100k_base approximation)
    /// instead of the naive `chars / 4`.
    pub fn calculate_total_tokens(&self, messages: &[ApiMessage], system_prompt: &str) -> u64 {
        let mut total = count_tokens(system_prompt) as u64;

        if let Some(ref structural) = self.structural_context {
            total += count_tokens(&structural.overview) as u64;
            total += count_tokens(&structural.dependencies) as u64;
            for file in &structural.key_files {
                total += count_tokens(file) as u64;
            }
        }
    ...
    fn update_token_count(&mut self, messages: &[ApiMessage], system_prompt: &str) {
        if self.api_prompt_tokens > 0 {
            ...
            if new_msg_count > 0 {
                ...
                // Only count new messages for delta, prompt/structural context included in baseline
                let delta = self.calculate_total_tokens(&messages[start..], "");
                self.last_token_count = self.api_prompt_tokens + delta;
            } else {
                self.last_token_count = self.api_prompt_tokens;
            }
        } else {
            self.last_token_count = self.calculate_total_tokens(messages, system_prompt);
        }
    }
}
