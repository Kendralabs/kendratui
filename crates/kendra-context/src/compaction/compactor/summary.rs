...
impl ContextCompactor {
    ...
    pub fn sliding_window_compact(&mut self, messages: Vec<ApiMessage>) -> Vec<ApiMessage> {
        ...
        if let Some(ref structural) = self.structural_context {
            ...
            full_summary.push_str(&structural.overview);
            full_summary.push_str("\n\n## Dependencies\n");
            full_summary.push_str(&structural.dependencies);
        }
    ...
    ///
    /// Note: artifact summary and structural context are NOT included here —
    /// callers append them separately.
    pub fn fallback_summary(&self, messages: &[ApiMessage]) -> String {
        ...
        for msg in messages {
            ...
            match role {
                "user"
                    if goal.is_empty()
                        && !kendra_models::message::is_system_injected_content(&content) =>
                {
    ...
    pub fn build_compaction_payload(
        ...
        if let Some(ref structural) = self.structural_context {
            ...
            final_system_prompt.push_str(&structural.overview);
        }
